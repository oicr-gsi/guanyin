...
describe('report_record', function() {
  ...
  it('should add a SINGLE report record on /reportdb/record POST', function(done) {
    chai.request(server)
      ...
      .send({
       ...
       "notification_message": "Here is the report: https://www.hpc.oicr.on.ca/archive/web/runReports/180109_D00331_0293_BCC5BJANXX/180109_D00331_0293_BCC5BJANXX_report.html",
  ...
  it('should update a SINGLE report record on /reportdb/record_notification?report_record_id={report_record_id}&notification_done={notification_done} PUT', function(done) {
    chai.request(server)
      .put('/reportdb/record_notification?report_record_id=1&notification_done=true')
      .end(function(err, res){
        res.should.have.status(200);
        res.should.be.json;
        res.body.should.be.a('object');
